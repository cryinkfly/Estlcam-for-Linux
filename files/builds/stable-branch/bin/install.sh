...
############################################################################################################################
# Name:         Estclam - Installationsskript (Linux)                                                                      #
# Description:  With this file you can install Estclam on different Linux distributions.                                   #
# Author:       Steve Zabka                                                                                                #
...
# Default-Path:
SP_PATH="$HOME/.estclam"

# Reset the logfile-value for the installation of Estclam.
...
# Check if the package "dialog", "wget" ... is installed on your system.
...
SP_REQUIRED_COMMANDS=("dialog" "wget" "lsb-release" "cat")
    for cmd in "${SP_REQUIRED_COMMANDS[@]}"; do
        ...
        if [ -n "${path}" ]; then
            ...
            read -p "Would you like to install these packages on your system to continue the installation of Estclam? (y/n)" yn
            case $yn in 
...
# Create a base structure for the installation of Estclam.
...
function SP_LOCALE_FILES {   
    wget -N -P "$SP_PATH/locale/en-US" --progress=dot "https://github.com/cryinkfly/Estlcam-for-Linux/raw/main/files/builds/stable-branch/locale/en-US/locale-en.sh" 2>&1 |\
    grep "%" |\
    ...
    sleep 1
    wget -N -P "$SP_PATH/locale/en-US" --progress=dot "https://github.com/cryinkfly/Estlcam-for-Linux/raw/main/files/builds/stable-branch/locale/en-US/license-en.txt" 2>&1 |\
    grep "%" |\
    sed -u -e "s,\.,,g" | awk '{print $2}' | sed -u -e "s,\%,,g"  | dialog --gauge "Download Locale files (EN) ..." 10 100
    sleep 1
    wget -N -P "$SP_PATH/locale/de-DE" --progress=dot "https://github.com/cryinkfly/Estlcam-for-Linux/raw/main/files/builds/stable-branch/locale/de-DE/locale-de.sh" 2>&1 |\
    grep "%" |\
    ...
    sleep 1
    wget -N -P "$SP_PATH/locale/en-US" --progress=dot "https://github.com/cryinkfly/Estlcam-for-Linux/raw/main/files/builds/stable-branch/locale/de-DE/license-de.txt" 2>&1 |\
    grep "%" |\
    sed -u -e "s,\.,,g" | awk '{print $2}' | sed -u -e "s,\%,,g"  | dialog --gauge "Download Locale files (DE) ..." 10 100
    sleep 1
}
...
function SP_SELECT_ESTCLAM_VERSION {
    if [[ $SP_ESTCLAM_VERSION = "02" ]]; then
    SP_SELECT_ESTCLAM
    else
    SP_ESTCLAM_LOAD
    fi
    SP_SELECT_ESTCLAM_VERSION
}

function SP_SELECT_ESTCLAM {
    ESTCLAM_EXE=$(dialog --backtitle "Setup - Estclam for Linux [Build Version 0.0.1]" \
    --title "$SP_SELECT_ESTCLAM_SUBTITLE" \
    --stdout --title "$SP_SELECT_ESTCLAM_TEXT" \
...
function SP_SOLIDWORKS_LOAD {
  ...
  sleep 1
  mv "Estclam.exe" "$SP_PATH/downloads/Estclam.exe"
  ESTCLAM_EXE="$SP_PATH/downloads/Estlcam.exe"
  SP_SELECT_OS_VERSION
}
...
function SP_ESTCLAM_INSTALL {
   cd "$SP_PATH/bin/winetricks"
   WINEPREFIX=$SP_PATH/wineprefixes/estclam sh winetricks -q win10 &&
   WINEPREFIX=$SP_PATH/wineprefixes/estclam wine $ESTCLAM_EXE &&
   SP_EXIT
}
...
function SP_WELCOME {
  SP_LOCALE=$(dialog --backtitle "Setup - Estclam for Linux [Build Version 0.0.1]" \
    --title "Welcome to the Estclam for Linux Installer Wizard" \
...
function SP_SHOW_LICENSE {
  dialog --backtitle "Setup - Estclam for Linux [Build Version 0.0.1]" \
  --yesno "`cat $SP_LICENSE_FILE`" 0 0
  response=$?
  case $response in
     0) SP_SELECT_ESTCLAM_VERSION;; # Open the next dialog window for selecting the correct Estclam Version.
...
function SP_SELECT_ESTCLAM_VERSION {
  SP_SOLIDWORKS_VERSION=$(dialog --backtitle "Setup - Estclam for Linux [Build Version 0.0.1]" \
    --title "$SP_SELECT_ESTCLAM_VERSION_SUBTITLE" \
    --radiolist "$SP_SELECT_ESTCLAM_VERSION_TEXT" 0 0 0 \
       01 "Estclam Version (11244)" on\
       02 "Estclam Version (11244), ..." off 3>&1 1>&2 2>&3 3>&-)
  clear
  echo "$SP_ESTCLAM_VERSION" >> $SP_PATH/cache/settings.txt
  SP_SELECT_ESTCLAM_VERSION
}
...
function SP_SELECT_OS_VERSION {
  SP_OS_VERSION=$(dialog --backtitle "Setup - Estclam for Linux [Build Version 0.0.1]" \
